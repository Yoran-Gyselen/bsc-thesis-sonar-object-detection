...
def get_model(trained_model_path, num_classes, device):
    # Load BYOL model
    byol_ckpt = torch.load(trained_model_path)
    byol_resnet_state_dict = {
        k.replace("online_encoder.", ""): v
        for k, v in byol_ckpt.items()
        if "online_encoder" in k and "fc" not in k  # Ignore classification head
    }

    # Load a fresh ResNet18 backbone
    resnet = resnet18()

    # Load the BYOL weights into this ResNet18
    resnet.load_state_dict(byol_resnet_state_dict, strict=False)

    # Remove the final fully connected layer
    resnet = nn.Sequential(*list(resnet.children())[:-2])  # Remove avgpool and fc

    # Use the modified ResNet18 as the backbone for the FPN
    backbone = resnet_fpn_backbone(resnet)

    # Create the Faster R-CNN model with the FPN backbone
